...
int main(int argc, const char* argv[]) {
    ...
    std::vector<uint8_t*> jpeg_pointers(NUM_KERNEL, nullptr);
    std::cout << "INFO: Created jpeg_pointers!\n";

#ifndef _HLS_TEST_
    //ap_uint<64>* yuv_mcu_pointer = aligned_alloc<ap_uint<64> >(sizeof(ap_uint<64>) * MAXCMP_BC * 8);
    
    // Create a vector to hold the pointers of type ap_uint<64>*
    std::vector<ap_uint<64>*> yuv_mcu_pointers(NUM_KERNEL, nullptr);
    std::cout << "INFO: Created yuv_mcu_pointers!\n";

    try {
        // Allocate memory for each pointer in the vector
        for (auto& pointer : yuv_mcu_pointers) {
            pointer = aligned_alloc<ap_uint<64>>(sizeof(ap_uint<64>) * MAXCMP_BC * 8);
            std::cout << "INFO: Created yuv_mcu_pointers element pointers!\n";
        }
    }
    ...
    std::vector<ap_uint<32>*> infoses(NUM_KERNEL, nullptr);
    std::cout << "INFO: Created infoses!\n";
    try {
        // Allocate memory for each pointer in the vector
        for (auto& pointer : infoses) {
            pointer = aligned_alloc<ap_uint<32>>(sizeof(ap_uint<32>) *1024);
            std::cout << "INFO: Created infoses element pointers!\n";
        }
    }
#else
    ...
    // Load data into each jpeg_pointer
    for (auto& jpeg_pointer : jpeg_pointers) {
        int err = load_dat(jpeg_pointer, JPEGFile, size);
        std::cout << "INFO: loaded data into jpeg_pointers!\n";
        if (err) {
            std::printf("Alloc buf failed!, size:%zu Bytes\n", size);
            return err;
        }
    }
    ...
    std::string krnl_name = "kernelJpegDecoder";
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
        ...
        std::string krnl_name_full = krnl_name + ":{" + "kernelJpegDecoder_" + cu_id + "}"
...
/*#ifndef USE_HBM
    ...
    mext_in[2].param = 0;
#else*/
    std::vector<cl_mem_ext_ptr_t> inbufext_jpeg(NUM_KERNEL);
    std::vector<cl_mem_ext_ptr_t> outbufext_yuv(NUM_KERNEL);
    std::vector<cl_mem_ext_ptr_t> outbufext_infos(NUM_KERNEL);

    for (int i = 0; i < NUM_KERNEL; i++) {
        inbufext_jpeg[i].flags = pc[i * 3];
        inbufext_jpeg[i].obj = jpeg_pointers[i];
        inbufext_jpeg[i].param = 0;

        outbufext_yuv[i].flags = pc[(i * 3) + 1];
        outbufext_yuv[i].obj = yuv_mcu_pointers[i];
        outbufext_yuv[i].param = 0;

        outbufext_infos[i].flags = pc[(i * 3) + 2];
        outbufext_infos[i].obj = infoses[i];
        outbufext_infos[i].param = 0;
    ...
    std::vector<cl::Buffer> buffer_input_jpeg(NUM_KERNEL);
    std::vector<cl::Buffer> buffer_output_yuv(NUM_KERNEL);
    std::vector<cl::Buffer> buffer_output_infos(NUM_KERNEL);

    for (int i = 0; i < NUM_KERNEL; i++) {
        buffer_input_jpeg[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                           sizeof(uint8_t) * (size), &inbufext_jpeg[i]);

        buffer_output_yuv[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY,
                           sizeof(ap_uint<64>) * MAXCMP_BC * 8, &outbufext_yuv[i]);

        buffer_output_infos[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY,
                           sizeof(ap_uint<32>) * 1024, &outbufext_infos[i]);
    ...
    */
    ...
    std::vector<std::vector<cl::Memory>> init_vectors(NUM_KERNEL);

    for (int i = 0; i < NUM_KERNEL; i++) {
        std::vector<cl::Memory>& init = init_vectors[i];
        init.push_back(inbufext_jpeg[i]);
        init.push_back(outbufext_yuv[i]);
        init.push_back(outbufext_infos[i]);
    }

    for (int i = 0; i < NUM_KERNEL; i++) {
    // migrate data from host to device
    q.enqueueMigrateMemObjects(init_vectors[i], CL_MIGRATE_MEM_OBJECT_CONTENT_UNDEFINED, nullptr, nullptr);
    ...
    std::vector<std::vector<cl::Memory>> ob_ins(NUM_KERNEL);
    std::vector<std::vector<cl::Memory>> ob_outs(NUM_KERNEL);

    for (int i = 0; i < NUM_KERNEL; i++){
        std::vector<cl::Memory> ob_in = ob_ins[i];
        std::vector<cl::Memory> ob_out = ob_outs[i];

        ob_in.push_back(buffer_input_jpeg[i]);
        ob_out.push_back(buffer_output_yuv[i]);
        ob_out.push_back(buffer_output_infos[i]);
    }
    ...
    for (int i = 0; i < NUM_KERNEL; i++){
        krnls[i].setArg(0, buffer_input_jpeg[i]);
        krnls[i].setArg(1, size);
        krnls[i].setArg(2, buffer_output_yuv[i]);
        krnls[i].setArg(3, buffer_output_infos[i]);
    }
    ...
    */
    ...
    // Create vectors to hold events for each kernel and each run
    std::vector<std::vector<std::vector<cl::Event>>> events_write(NUM_KERNEL);
    std::vector<std::vector<std::vector<cl::Event>>> events_kernel(NUM_KERNEL);
    std::vector<std::vector<std::vector<cl::Event>>> events_read(NUM_KERNEL);

    // Initialize the vectors
    for (int k = 0; k < NUM_KERNEL; ++k) {
        events_write[k].resize(num_runs);
        events_kernel[k].resize(num_runs);
        events_read[k].resize(num_runs);

        for (int i = 0; i < num_runs; ++i) {
            events_write[k][i].resize(1);
            events_kernel[k][i].resize(1);
            events_read[k][i].resize(1);
        }
    }
    ...
    for (int i = 0; i < NUM_KERNEL; ++i) {
        for (int j = 0; j < num_runs; ++j) {
            if (j < 1) {
                q.enqueueMigrateMemObjects(ob_ins[i], 0, nullptr, &events_write[i][j][0]); // 0 : migrate from host to dev
            } else {
                q.enqueueMigrateMemObjects(ob_ins[i], 0, &events_read[i][j - 1],
                                           &events_write[i][j][0]); // 0 : migrate from host to dev
            }
            // Launch kernel and compute kernel execution time
            q.enqueueTask(krnls[i], &events_write[i][j], &events_kernel[i][j][0]);
    
            // Data transfer from device buffer to host buffer
            q.enqueueMigrateMemObjects(ob_outs[i], 1, &events_kernel[i][j], &events_read[i][j][0]); // 1 : migrate from dev to host
        }
    }
    q.finish();
    ...
    std::cout << "-------------------------------------------------------" << std::endl;
    events_write[0][0][0].getProfilingInfo(CL_PROFILING_COMMAND_START, &timeStart);
    events_write[0][0][0].getProfilingInfo(CL_PROFILING_COMMAND_END, &timeEnd);
    write_time = (timeEnd - timeStart) / 1000.0;
    std::cout << "INFO: Data transfer from host to device: " << write_time << " us\n";
    std::cout << "-------------------------------------------------------" << std::endl;
    events_read[0][0][0].getProfilingInfo(CL_PROFILING_COMMAND_START, &timeStart);
    events_read[0][0][0].getProfilingInfo(CL_PROFILING_COMMAND_END, &timeEnd);
    ...
    for (int j = 0; j < NUM_KERNEL; ++j) {
        for (int i = 0; i < num_runs; ++i) {
            events_kernel[j][i][0].getProfilingInfo(CL_PROFILING_COMMAND_START, &timeStart);
            events_kernel[j][i][0].getProfilingInfo(CL_PROFILING_COMMAND_END, &timeEnd);
